...
void main()
{
    ...
    vec4 ref = vec4(r[0],r[1],r[2],0);

    diffuse = gl_LightSource[0].diffuse * gl_FrontMaterial.diffuse * float(max(0.0,float(dot(normal,light_direction))));
    specular = gl_LightSource[0].specular * gl_FrontMaterial.specular * float(pow(float(max(0.0,dot(position,ref))),gl_FrontMaterial.shininess));    
